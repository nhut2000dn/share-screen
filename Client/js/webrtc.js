...
socket.on('connect', function() {
  ...
  socket.on(SHARE_SCREEN_EVENT, async (description) => {
    console.log(OFFER_EVENT, description);
    alert("SHARE_SCREEN_EVENT")
  });
...
function emitShareScreenEvent(peerId, description) {
  if (socket && socket.connected) {
    socket.emit(SHARE_SCREEN_EVENT, { peerId: peerId, description: description });
  }
}

function emitShareScreenEvent(description) {
  if (socket && socket.connected) {
...
async function onCreateShareScreenSuccess(desc) {
  try {
    await peerConnection.setLocalDescription(desc);
    console.log(`setLocalDescription complete`);
    emitShareScreenEvent(calleeId, desc);
  } catch (e) {
    onSetSessionDescriptionError(e);
  }
}
